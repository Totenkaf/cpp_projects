...
public:
	...
	void ChangeFirstName(int year, const string& first_name) {
		// äîáàâèòü ôàêò èçìåíåíèÿ èìåíè íà first_name â ãîä year
		for (auto& i : m) {
			if (year > i.first) {
	...
	void ChangeLastName(int year, const string& last_name) {
		// äîáàâèòü ôàêò èçìåíåíèÿ ôàìèëèè íà last_name â ãîä year
		for (auto& i : m) {
			if (year > i.first) {
	...
	string  GetFullName(int year) const {
		...
		// ïîëó÷èòü èìÿ è ôàìèëèþ ïî ñîñòîÿíèþ íà êîíåö ãîäà year
		int i = 0;
		string first = "";
		string last = "";
		for (const auto& y : m) {
